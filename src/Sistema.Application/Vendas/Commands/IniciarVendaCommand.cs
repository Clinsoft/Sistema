...
public record IniciarVendaCommand(Guid EmpresaId, Guid UsuarioId, Guid LocalEstoqueId, Guid? ClienteId = null)
    : IRequest<Guid>;
    ...
    : IRequestHandler<IniciarVendaCommand, Guid>
{
    public async Task<Guid> Handle(IniciarVendaCommand cmd, CancellationToken ct)
    {
        ...
        var venda = Venda.Iniciar(cmd.EmpresaId, cmd.UsuarioId, cmd.LocalEstoqueId, numero, cmd.ClienteId);
